import { GOODS, GOOD_TYPES, getValidGoods, getValidGoodTypes } from './sheriffGoods.js';
import { Player } from './sheriffPlayer.js';
...
// Load table once DOM in scorer.html is ready
document.addEventListener("DOMContentLoaded", loadPage);

/**
 * In the three-player game, some goods have fewer cards, and some are missing entirely.
 * If the maximum has been set to 0, remove the whole row, as it will not be used.
 */
function removeUnusedGoods() {
  GOODS.forEach((good) => {
    if (!good.isValid()) {
      const unusedRow = document.getElementById(good.name);
      unusedRow.parentNode.removeChild(unusedRow);
    }
  });

  GOOD_TYPES.forEach((goodType) => {
    if (!goodType.isValid()) {
      const unusedRow = document.getElementById(goodType.name);
      unusedRow.parentNode.removeChild(unusedRow);
    }
  });
}
...
document.getElementById('calculateButton').addEventListener('click', calculateTotals);

/**
 * Calculate and log subtotals, then populate fields with totals. Fired when button clicked on scorer.html
 */
function calculateTotals() {
  ...
  getValidGoodTypes().forEach((goodType) => {
    ...
    if (kings.length === 1) {
      ...
      const bonus = queens.length > 1 ? Math.floor(goodType.queenBonus / queens.length) : goodType.queenBonus;
  ...
  players.forEach((player) => {
    getValidGoodTypes().forEach((goodType) => {
      ...
      document.getElementById(`${goodType.name}${player.key}`).innerHTML = royalty || '';
    });
  });
...
// Set up the page. Called once scorer.html DOM is loaded
  ...
  if (confirm('"Sheriff of Nottingham" is a game published by Cool Mini or Not. It is fun to play, but a pain to score. This page helps players score the game.\n\nPress "OK" to start entering player names. Press "reload" in your browser to reset the page.')) {
    ...
    players.forEach((player) => {
      ...
      getValidGoodTypes().forEach((goodType) => {
        ...
        bonusCell.id = `${goodType.name}${player.key}`;
        document.getElementById(goodType.name).appendChild(bonusCell);
      });
